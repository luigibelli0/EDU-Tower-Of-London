"""DEPRECATION WARNING
This legacy testing file creator script (create_testing_files.py) is deprecated and kept
only for backward compatibility with older release workflows (create_testing_files.yml).
Prefer the new Bedrock / Education pipelines controlled via 'enable_bedrock_pipeline'
and 'enable_edu_pipeline' in pack/release_config.json. This script will be removed in a
future cleanup. Do not extend it further.
"""
...
MCTEMPLATE_FILE_ROOT = sys.argv[3]
...
def main():
    print("DEPRECATION WARNING: 'create_testing_files.py' is a legacy script and will be removed once the new Bedrock / Education pipelines are fully adopted.", file=sys.stderr)
    print("create_testing_files.py: Script started")
    zip_path = Path(ZIP_FILES_PATH)
    root_path = Path(ROOT_PATH)
    mctemplate_file_root = Path(MCTEMPLATE_FILE_ROOT)
    ...
    if keyart_path is not None:
        img = Image.open(root_path / keyart_path).convert("RGB")
        img = img.resize((800, 450), resample=Image.BILINEAR)
        img.save(zip_path / "world_icon.jpeg")

    mctemplate_file_path = (
        mctemplate_file_root / f"{product_creator}_{product_key}_{ZIP_FILE_SUFFIX}"
    )
    print(
        f"package_release.py: Creating mctemplate file at {mctemplate_file_path.as_posix()}"
    )
    mctemplate_file_root.mkdir(exist_ok=True, parents=True)

    shutil.make_archive(mctemplate_file_path, "zip", zip_path)
    shutil.move(
        f"{mctemplate_file_path.as_posix()}.zip",
        f"{mctemplate_file_path.as_posix()}.mctemplate",
    )
