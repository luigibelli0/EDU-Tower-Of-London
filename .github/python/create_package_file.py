"""DEPRECATION WARNING
This legacy packaging script (create_package_file.py) is deprecated and kept only for
backward compatibility with older release workflows (create_package_file.yml).
New Bedrock and Education release pipelines should be used instead (those governed by
the 'enable_bedrock_pipeline' and 'enable_edu_pipeline' flags in pack/release_config.json).
This script will be removed in a future cleanup. Avoid adding new features here.
"""
...
def main():
    print("DEPRECATION WARNING: 'create_package_file.py' is a legacy script and will be removed once the new Bedrock / Education pipelines are fully adopted.", file=sys.stderr)
    print("create_packing_file.py: Script started")
    ...
    print(f"package_release.py: Creating path '{zip_world.as_posix()}'")
    zip_world.mkdir(exist_ok=True, parents=True)

    print(f"package_release.py: Creating path '{zip_store.as_posix()}'")
    zip_store.mkdir(exist_ok=True, parents=True)

    print(f"package_release.py: Creating path '{zip_marketing.as_posix()}'")
    ...
    world_path = Path(root_path / "regolith" / "worlds" / config["exported_world_bedrock"])
    ...
    if use_placeholder_marketing:
        ...
        # Copy Store Art
        # if screenshot*.png -> (Store Art & Marketing Art)
        for screenshot in (root_path / "pack").glob("screenshot_*.png"):
            try:
                index = int(screenshot.stem.split("_")[-1])
            except:
                continue
            ...
            img = Image.open(screenshot).convert("RGB")  # There is no RGBA .jpg-s
            img = img.resize((800, 450), resample=Image.LANCZOS)
            img.save(zip_store / f"{product_key}_Screenshot_{index}.jpg", quality=100, subsampling=0, optimize=False)
        # if screenshot*.jpg -> (Store Art & Marketing Art)
        for screenshot in (root_path / "pack").glob("screenshot_*.jpg"):
            try:
                index = int(screenshot.stem.split("_")[-1])
            except:
                continue
        ...
        # panorama.jpg
        shutil.copy(root_path / "pack/panorama.jpg", zip_store / "panorama_0.jpg")
        # keyart.png
    ...
    if use_placeholder_marketing:
        zip_file_path = (
            zip_file_root / f"PLACEHOLDER_DO_NO_USE_{product_key}_{ZIP_FILE_SUFFIX}"
        )
    else:
        zip_file_path = (
            zip_file_root / f"{product_creator}_{product_key}_{ZIP_FILE_SUFFIX}"
        )

    print(f"package_release.py: Creating zip file at {zip_file_path.as_posix()}")
