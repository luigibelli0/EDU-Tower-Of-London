...
def main():
    print("create_package_file_bedrock.py: Script started")
    ...
    print(f"create_package_file_bedrock.py: Creating path '{zip_world.as_posix()}'")
    zip_world.mkdir(exist_ok=True, parents=True)

    print(f"create_package_file_bedrock.py: Creating path '{zip_store.as_posix()}'")
    zip_store.mkdir(exist_ok=True, parents=True)

    print(f"create_package_file_bedrock.py: Creating path '{zip_marketing.as_posix()}'")
    ...
    # Use Bedrock-specific configuration
    product_creator = config["product_creator"]
    product_name = config.get("product_name", config.get("product_name", ""))
    product_key = config["product_key"]
    product_description = config["product_description"]
    exported_world = config.get("exported_world_bedrock")
    ...
    if use_placeholder_marketing:
        zip_file_path = (
            zip_file_root / f"PLACEHOLDER_DO_NOT_USE_{product_key}_Bedrock_{ZIP_FILE_SUFFIX}"
        )
    else:
        zip_file_path = (
            zip_file_root / f"{product_creator}_{product_key}_Bedrock_{ZIP_FILE_SUFFIX}"
        )

    print(f"create_package_file_bedrock.py: Creating zip file at {zip_file_path.as_posix()}")
    zip_file_root.mkdir(exist_ok=True, parents=True)

    shutil.make_archive(zip_file_path, "zip", zip_path)
    print("create_package_file_bedrock.py: Finished with no errors!")
